...
def callback(ch, method, properties, body):
	...
	ch.basic_publish(exchange='', routing_key=properties.reply_to, properties=pika.BasicProperties(correlation_id = properties.correlation_id), body=str('got ya back'))
	ch.basic_ack(delivery_tag = method.delivery_tag)
	print " [x] Done, method.delivery_tag = " + str(method.delivery_tag)


channel.basic_consume(callback, queue='task_q')
