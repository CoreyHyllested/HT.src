...
giftcertificate = Table('giftcertificate', post_meta,
    ...
    Column('gift_recipient_name', String(length=64), nullable=False),
    Column('gift_recipient_mail', String(length=64)),
...
def upgrade(migrate_engine):
    ...
    post_meta.tables['giftcertificate'].columns['gift_recipient_name'].create()
    post_meta.tables['giftcertificate'].columns['gift_recipient_note'].create()
    post_meta.tables['giftcertificate'].columns['gift_recipient_prof'].create()
    post_meta.tables['giftcertificate'].columns['gift_recipient_proj'].create()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    post_meta.tables['giftcertificate'].columns['gift_recipient_name'].drop()
    post_meta.tables['giftcertificate'].columns['gift_recipient_note'].drop()
    post_meta.tables['giftcertificate'].columns['gift_recipient_prof'].drop()
    post_meta.tables['giftcertificate'].columns['gift_recipient_proj'].drop()
